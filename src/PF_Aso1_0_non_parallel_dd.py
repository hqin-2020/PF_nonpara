...
def decompose_θ(θ):
    ...
    return λ, η, b11, b22, As11, As22, As33, Bs11, Bs22, Bs33
...
def init_H(Λ_scale, cd_scale):
    ...
    H0 = {'1st':[np.array([[1], [1]]), np.eye(2) * Λ_scale, 1 * cd_scale, 1 * cd_scale], 
          ...
          '4th':[np.array([[0]]), np.eye(1) * Λ_scale, 1 * cd_scale, 1 * cd_scale],
          '5th':[np.array([[0]]), np.eye(1) * Λ_scale, 1 * cd_scale, 1 * cd_scale]}
    
    return H0

def init_X(θ, D_0):
    
    λ, η, b11, b22, As11, As22, As33, Bs11, Bs22, Bs33 = decompose_θ(θ)
    ones = np.ones([3,1])
...
def update_H(Xt_next, Xt, Ht):
    ...
    bayes_para_update(Ht['4th'][0], Ht['4th'][1], Ht['4th'][2], Ht['4th'][3], np.array([[St2]]), St_next_2)
    
    fifth_eq_bt_next, fifth_eq_Λt_next, fifth_eq_ct_next, fifth_eq_dt_next = \
    bayes_para_update(Ht['5th'][0], Ht['5th'][1], Ht['5th'][2], Ht['5th'][3], np.array([[St3]]), St_next_3)
