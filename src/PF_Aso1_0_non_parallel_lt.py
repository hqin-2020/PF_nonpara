...
def decompose_θ(θ):
    ...
    As11 = θ['Ass'][0,0]
    As21 = θ['Ass'][1,0]; As22 = θ['Ass'][1,1]
    As31 = θ['Ass'][2,0]; As32 = θ['Ass'][2,1]; As33 = θ['Ass'][2,2]
    Bs11 = θ['Bs'][0,0];  Bs22 = θ['Bs'][1,1]; Bs33 = θ['Bs'][2,2]

    return λ, η, b11, b22, As11, As21, As22, As31, As32, As33, Bs11, Bs22, Bs33
...
def update_θ(H):
    ...
    while Ass_stable == False:
        ...
        Ass = np.array([[β_s1[0,0], 0, 0],\
                        [β_s2[0,0], β_s2[1,0], 0],\
                        [β_s3[0,0], β_s3[1,0], β_s3[2,0]]])
        Ass_iter += 1
    ...
    Bs = np.diag([σ2_s1,σ2_s2,σ2_s3])**0.5
    θ = {'Azo' : Azo, 'Azz' : Azz, 'Bz' : Bz, 'Ass' : Ass, 'Bs' : Bs, 'λ_iter': λ_iter, 'Ass_iter' : Ass_iter}
...
def init_H(Λ_scale, cd_scale):
    ...
    H0 = {'1st':[np.array([[1], [1]]), np.eye(2) * Λ_scale, 1 * cd_scale, 1 * cd_scale], 
          ...
          '3rd':[np.array([[0]]), np.eye(1) * Λ_scale, 1 * cd_scale, 1 * cd_scale], 
          '4th':[np.array([[0], [0]]), np.eye(2) * Λ_scale, 1 * cd_scale, 1 * cd_scale],
          '5th':[np.array([[0], [0], [0]]), np.eye(3) * Λ_scale, 1 * cd_scale, 1 * cd_scale]}
    
    return H0

def init_X(θ, D_0):
    
    λ, η, b11, b22, As11, As21, As22, As31, As32, As33, Bs11, Bs22, Bs33 = decompose_θ(θ)
    ones = np.ones([3,1])
    Ass = np.array([[As11, 0,    0],\
                    [As21, As22, 0],\
                    [As31, As32, As33]])
    ...
    μs = sp.linalg.solve(np.eye(3) - Ass, np.zeros([3,1])) 
...
def update_X(Dt_next, Xt, θt_next):
    ...
    mean = np.vstack([Azo + Azz@Zt + Φ.T@(Dt_next-ones*Zt1 - ones*Zt2 - Ass@St),\
                      Ass@St + Γ.T@(Dt_next-ones*Zt1 - ones*Zt2 - Ass@St)])
...
def update_H(Xt_next, Xt, Ht):
    ...
    bayes_para_update(Ht['3rd'][0], Ht['3rd'][1], Ht['3rd'][2], Ht['3rd'][3], np.array([[St1]]), St_next_1)
    
    fourth_eq_bt_next, fourth_eq_Λt_next, fourth_eq_ct_next, fourth_eq_dt_next = \
    bayes_para_update(Ht['4th'][0], Ht['4th'][1], Ht['4th'][2], Ht['4th'][3], np.array([[St1],[St2]]), St_next_2)
    
    fifth_eq_bt_next, fifth_eq_Λt_next, fifth_eq_ct_next, fifth_eq_dt_next = \
    bayes_para_update(Ht['5th'][0], Ht['5th'][1], Ht['5th'][2], Ht['5th'][3], np.array([[St1],[St2],[St3]]), St_next_3)
