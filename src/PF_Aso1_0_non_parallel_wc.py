...
def decompose_θ(θ):
    ...
    return λ, η, b11, b22, As11, As12, As13, As21, As22, As23, As31, As32, As33, Bs11, Bs21, Bs22, Bs31, Bs32, Bs33, j21, j31, j32
...
def update_θ(H):
    ...
    while Ass_stable == False:
        ...
        Ass = sp.linalg.solve(J_inv, np.array([[β_s1[0,0], β_s1[1,0], β_s1[2,0]],\
                                               [β_s2[1,0], β_s2[2,0], β_s2[3,0]],\
                                               [β_s3[2,0], β_s3[3,0], β_s3[4,0]]]))
        Ass_iter += 1
    ...
    θ = {'Azo' : Azo, 'Azz' : Azz, 'Bz' : Bz, 'Ass' : Ass, 'Bs' : Bs, 'J_inv': J_inv, 'λ_iter': λ_iter, 'Ass_iter' : Ass_iter}
...
def init_H(Λ_scale, cd_scale):
    ...
    H0 = {'1st':[np.array([[1], [1]]), np.eye(2) * Λ_scale, 1 * cd_scale, 1 * cd_scale], 
          ...
          '4th':[np.array([[0], [0], [0], [0]]), np.eye(4) * Λ_scale, 1 * cd_scale, 1 * cd_scale],
          '5th':[np.array([[0], [0], [0], [0], [0]]), np.eye(5) * Λ_scale, 1 * cd_scale, 1 * cd_scale]}
    
    return H0

def init_X(θ, D_0):
    
    λ, η, b11, b22, As11, As12, As13, As21, As22, As23, As31, As32, As33, Bs11, Bs21, Bs22, Bs31, Bs32, Bs33, j21, j31, j32 = decompose_θ(θ)
    ones = np.ones([3,1])
...
def update_H(Xt_next, Xt, Ht):
    ...
    bayes_para_update(Ht['4th'][0], Ht['4th'][1], Ht['4th'][2], Ht['4th'][3], np.array([[St_next_1],[St1],[St2],[St3]]), St_next_2)
    
    fifth_eq_bt_next, fifth_eq_Λt_next, fifth_eq_ct_next, fifth_eq_dt_next = \
    bayes_para_update(Ht['5th'][0], Ht['5th'][1], Ht['5th'][2], Ht['5th'][3], np.array([[St_next_1],[St_next_2],[St1],[St2],[St3]]), St_next_3)
